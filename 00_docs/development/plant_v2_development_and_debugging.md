...
包含 V2 独有的 Adapter 和组合根。当前 VS Code 任务仍默认生成 V1 数据库，开发 V2 时要
显式传入 `PLANT_PRODUCT=v2`。
...
传感器抗干扰。当前 V2 要求所有启动传感器有效，因此裸板显示 `status=fault` 是当前实现
的预期结果，但“任一非关键传感器坏掉就整机 Fault”是否适合用户仍需产品决策。
...
- clangd 的 VS Code 默认任务仍生成 V1 数据库，V2 开发者需要手动选择产品；后续应增加
  明确的 V1/V2 任务或可输入的产品参数。
- 非关键环境传感器启动失败目前会让整机进入 `Fault`，需要定义“关键故障/功能降级”矩阵。
- BLE 在拥挤环境中的发现时延、连接重试和配对窗口尚未形成产品级策略。
- 深度睡眠默认值尚未用整机电流与用户重连时延共同验证。
