...
最多保存 3 个绑定且同时只连接 1 台主机。已加密且已绑定的主机可发送 `ForgetBonds`
清除全部密钥，设备回复后重启；无法使用原主机时，持续触摸 TTP223 10 秒触发本地恢复。
擦除整片 Flash 或 NVS 也会丢失绑定，这属于显式恢复/重新烧录行为。
...
| `0x20` | ForgetBonds | 无 | 清除全部绑定并重启；仅加密链路可写 |
...
`--protocol-version 1`；解绑使用 `forget-bonds`，成功响应后设备会重启。
